...
class TestTree : public TreeMap<int, string> {
    protected:
        using Node = typename TreeMap<int, string>::Node;
    
        void inorder(Node* n) {
            if (!n || n == tree.rt) return;
            inorder(n->left);
            cout << n->element.first.key() << ' '
                 << n->element.first.value() << ' '
                 << n->element.second << '\n';
            inorder(n->right);
        }
    ...
    public:
        void print_tree() {
            inorder(tree.rt->left);  // skip the sentinel root
        }
    };
    ...
    int main() {
        ...
        // Search for 15, 30, and 8
        cout << (bst.find(15) != bst.end() ? "15 found\n" : "15 not found\n");
        cout << (bst.find(30) != bst.end() ? "30 found\n" : "30 not found\n");
        cout << (bst.find(8)  != bst.end() ? "8 found\n"  : "8 not found\n");
    
        // Final BST print (inorder)
        bst.print_tree();
    }