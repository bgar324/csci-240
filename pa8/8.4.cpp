...
private:
    ...
    using Node = typename AVLTreeMap<int,string>::Node;
    void draw(Node* n,int indent){
        if(!n) return;
        cout<<string(indent,' ')<<n->element.first.key()<<endl;
        draw(n->left,indent+2);
        draw(n->right,indent+2);
    }


public:
    BetterPopMap(string file)
    {
        ifstream ss(file);
        string str;
    ...
    void drawTree(){
        Node* root = map.tree.sentinel()->left;
        draw(root,0);
    }
};
