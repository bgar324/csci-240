...
public:
    ...
    int height()
    {
        return max(treeTras(tree.rt->left), treeTras(tree.rt->right));
    }

    int treeTras(Node *node)
    {
        if (node == nullptr)
            return -1;
        return 1 + max(treeTras(node->left), treeTras(node->right));
    }
};
...
public:
    SimplePopMap(string file)
    {
        ifstream ss(file);
        string str;
        getline(ss, str); // skip first line
    ...
    void find(int key)
    {
        ...
        if (it != map.end())
        {
            cout << "Key Found for " << it->value() << endl;
        }
        else
            cout << "Key Not Found\n";
    }
    ...
    void insert(int key, string str)
    {
        ...
        if (it != map.end())
        {
            cout << "Insertion Replaced: Key Found for " << it->value() << endl;
        }
        else
        {
            cout << "Key Inserted\n";
    ...
    void erase(int key)
    {
        ...
        if (it != map.end())
        {
            cout << "Key erased for " << it->value() << endl;
            map.erase(key);
        }
        else
            cout << "Erase failed, Key Not Found\n";
    }
...
int main() {
    SimplePopMap db("popSmall.txt");

    cout << "\n[1] Initial Records:" << endl;
    db.print();

    cout << "\n[2] Search for 6037:" << endl;
    db.find(6037);

    cout << "\n[3] Search for 6000:" << endl;
    db.find(6000);

    cout << "\n[4] Insert 6066, \"1, New County, CA\":" << endl;
    db.insert(6066, "1, New County, CA");

    cout << "\n[5] Insert 6065, \"2000, Riverside, CA\":" << endl;
    db.insert(6065, "2000, Riverside, CA");
