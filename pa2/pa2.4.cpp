  ...
  */
  ...
  void removeElem(string name){
    ...
    */
    ...
    if(head->name == name){ //ok so if the first node (head)'s name equates to name , then we should basically increment on the linked list, then set the head to be the temp. 
      Node* temp = head;
      head = head->next;
      delete temp;
      numElems--;
