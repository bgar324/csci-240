...
bool dfs_find_path(
    ...
    std::vector<typename Graph<V,E>::Vertex>& current,
    std::vector<typename Graph<V,E>::Vertex>& solution
) {
    visited.insert(u);
    current.push_back(u);

    if (u == dest) {
        // Found destination; copy the current recursion stack into solution
        solution = current;
        return true;
    }

    for (auto nbr : G.neighbors(u, /*outgoing=*/true)) {
        if (visited.count(nbr) == 0) {
            if (dfs_find_path(G, nbr, dest, visited, current, solution))
                return true; 
        }
    }
...
void path(
    ...
    bool found = dfs_find_path(G, src, dest, visited, current, solution);
    if (!found) {
        std::cout << "no\n";
    } else {
        std::cout << "yes (";
        for (size_t i = 0; i < solution.size(); ++i) {
            std::cout << *solution[i];           // print the integer label
...
int main() {
    // 1) Build a directed Graph<int,int> with vertices 0..4
    Graph<int,int> myGraph(true);      // “true” means “this is a digraph”

    // Insert vertices 0, 1, 2, 3, 4
    std::vector<Graph<int,int>::Vertex> V(5);
    for (int i = 0; i < 5; ++i) {
        V[i] = myGraph.insert_vertex(i);
    }

    // 2) Insert edges exactly as in the problem’s diagram:
    //    0 -> 2, 0 -> 3, 1 -> 3, 3 -> 2, 2 -> 4
    myGraph.insert_edge(V[0], V[2]);
    myGraph.insert_edge(V[0], V[3]);
    myGraph.insert_edge(V[1], V[3]);
    myGraph.insert_edge(V[3], V[2]);
    myGraph.insert_edge(V[2], V[4]);

    // 3) Call path(...) twice:
    std::cout << "path(myGraph, 0, 4): ";
    path(myGraph, V[0], V[4]);    // Expect “yes (0 3 2 4)” or “yes (0 2 4)”

    std::cout << "path(myGraph, 4, 0): ";
    path(myGraph, V[4], V[0]);    // Expect “no”
